...
def superimpose(img1, img2):
    ...
    for i in range(img1.shape[0]):
        for j in range(img1.shape[1]):
            if img1[i][j][3] == 0 and img2[i][j][3] == 0:
                output_img[i][j] = [255, 255, 255, 255]
            else:
                output_img[i][j][3] = 255
...
#img1 = cv2.imread('../greyscale/outputs/' + filename1)
#img2 = cv2.imread('../greyscale/outputs/' + filename2)
#
#superimposed = superimpose(img1, img2)
#
#cv2.imwrite('../greyscale/outputs/superimposed_output.png', superimposed)